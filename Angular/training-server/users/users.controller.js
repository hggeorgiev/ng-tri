...
function create(req,res,next) {
    userService.create(req.body).then(user => user ? res.json(user) : res.sendStatus(400))
        .catch(next);
}

function update(req,res,next) {
    userService.update(req.params.id, req.body,).then(user => user ? res.json(user) : res.sendStatus(400))
        .catch(next);
}

function remove(req,res,next) {
    //const decodedUser = userService.verifyJwtToken(req.headers.authorization)

    // if (decodedUser.id === req.params.id && req.user.id === req.params.id) {
    //     return res.status(401).json({ message: 'Unauthorized' });
    // }

    userService.remove(req.params.id).then(user => user ? res.json(user) : res.sendStatus(404))
        .catch(next);
}
