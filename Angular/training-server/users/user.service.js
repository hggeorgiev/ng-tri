...
async function create(details) {
    ...
    details.username = details.firstName.toLowerCase();
    details.passwordHash = bcrypt.hashSync('test', 10);
    const user = new db.User(details);
    await user.save()
    return basicDetails(user);
}
...
function verifyJwtToken(bearerToken) {
...
function basicDetails(user) {
    const { id, firstName, lastName, username, role, email } = user;
    return { id, firstName, lastName, username, role, email };
}
